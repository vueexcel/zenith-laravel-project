        ...
        foreach ($files as $each_file) {
            ...
            // try {
                foreach ($contents as $content) {
                    ...
                    if ($content['CaseLegacyId'] && HealthConcern::where('episode_reference', $content['CaseLegacyId'])->count() || $content['CaseReference'] && HealthConcern::where('episode_reference', strval($content['CaseReference']))->count()) {
                        
                        if ($content['CaseLegacyId'])
                            $health_concern = HealthConcern::where('episode_reference', $content['CaseLegacyId'])->first();
                        if (!$health_concern || !$content['CaseLegacyId']) {
                        ...
                        if ($health_concern) {
                            ...
                            isset($content['DateIssueFirstRaised']) && $content['DateIssueFirstRaised'] != 'Invalid date' && $health_concern->concern_date = $content['DateIssueFirstRaised'];
                            ...
                            isset($content['Origin']) && $health_concern->origin = str_replace(" Concern", "", $content['Origin']);
                            ...
                            isset($content['CurrentStatus']) && $health_concern->{"next_steps_{$max}"} = $this->changeCurrentStatus($content['CurrentStatus']);
                            ...
                            isset($content['AppointmentDateTime']) && $health_concern->{"next_steps_{$max}_date"} = substr($content['AppointmentDateTime'],0,10);
