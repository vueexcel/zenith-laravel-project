                ...
                foreach ($contents as $content) {
                    ...
                    if (($content['CaseLegacyId'] && HealthConcern::where('is_deleted', 0)->where('episode_reference', $content['CaseLegacyId'])->count()) || ($content['CaseReference'] && HealthConcern::where('is_deleted', 0)->where('episode_reference', strval($content['CaseReference']))->count())) {
                        ...
                        if ($health_concern) {
                            ...
                            } elseif (isset($content['NumberOfWeeksToBeInPlace'])){
                                $ramp_up = RampUp::where('ramp_up', $content['NumberOfWeeksToBeInPlace'] . ($content['NumberOfWeeksToBeInPlace'] == 1 ? " Week" : " Weeks"))->first();

                                if($ramp_up != null) {
                                    $health_concern->ramp_up = $content['NumberOfWeeksToBeInPlace'] . ($content['NumberOfWeeksToBeInPlace'] == 1 ? " Week" : " Weeks");
                                } else {
                                    RampUp::create([
                                        'ramp_up' => $content['NumberOfWeeksToBeInPlace'] . ($content['NumberOfWeeksToBeInPlace'] == 1 ? " Week" : " Weeks"),
                                    ]);
                                    $health_concern->ramp_up = $content['NumberOfWeeksToBeInPlace'] . ($content['NumberOfWeeksToBeInPlace'] == 1 ? " Week" : " Weeks");
                                }
                            }
                            ...
                            if (isset($content['Level']) && isset($content['AppointmentDateTime'])) {
                                if ((strtolower($content['Level']) == 'level1discharged') || (strtolower($content['Level']) == 'level2discharged') || (strtolower($content['Level']) == 'level3discharged') || (strtolower($content['Level']) == 'level4(redflex)discharged')){
                                    $health_concern->discharge_date = substr($content['AppointmentDateTime'],0,10);
                                }
                            }

                            for ($i = 1; $i < 9; $i++) {
                                if (!$health_concern->{"next_steps_{$i}"} && !$health_concern->{"next_steps_{$i}_date"} && isset($content['CurrentStatus'])) {
                                    $j = $i - 1;
                                    if ($this->changeCurrentStatus($content['CurrentStatus']) != $health_concern->{"next_steps_{$j}"}){
                                        ...
                                        isset($content['CurrentStatus']) && $this->changeCurrentStatus($content['CurrentStatus']) && isset($content['AppointmentDateTime']) && $health_concern->next_step_id = $this->changeCurrentStatus($content['CurrentStatus']);
                                        $health_concern = $this->regulate_steps($health_concern, $i);
                                        if(!empty($content['NumberOfWeeksToBeInPlace']) && !empty($content['AppointmentDateTime'])) {
                                            $ramp_up_week = '+'.$content['NumberOfWeeksToBeInPlace'].' weeks';
                                            $health_concern->fully_fit = date('Y-m-d', strtotime($ramp_up_week, \DateTime::createFromFormat("Y-m-d", $health_concern->{"next_steps_{$i}_date"})->getTimestamp()));
                                        }
                                        $health_concern->save();
                                        break;
                                    }else {
                                        isset($content['CurrentStatus']) && $this->changeCurrentStatus($content['CurrentStatus']) && isset($content['AppointmentDateTime']) && $health_concern->{"next_steps_{$j}_date"} = substr($content['AppointmentDateTime'],0,10);
                                        $health_concern->save();
                                    }
                                    break;
                                }
                        ...
                        isset($content['CurrentStatus']) && $health_concern->next_step_id = $this->changeCurrentStatus($content['CurrentStatus']);
                        ...
                        if (isset($content['OrgLevelTwo'])) {
                            ...
                            if(isset($content['AppointmentDateTime']) && !empty($content['NumberOfWeeksToBeInPlace']) && !empty($content['AppointmentDateTime'])) {
                                $ramp_up_week = '+'.$content['NumberOfWeeksToBeInPlace'].' weeks';
                                $health_concern->fully_fit = date('Y-m-d', strtotime($ramp_up_week, \DateTime::createFromFormat("Y-m-d", $health_concern->next_steps_1_date)->getTimestamp()));
                            }
                        $health_concern->save();
                    }
                }
