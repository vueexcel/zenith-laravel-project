...
class TargetController extends Controller
{
    ...
    public function index($report_type)
    {
        ...
        switch ($report_type) {
            case 'lt' : $data['sub_title'] = 'LT'; break;
            case 'accident' : $data['sub_title'] = 'Accidents'; break;
            case 'gir' : $data['sub_title'] = 'GIR'; break;
            case 'work_mss' : $data['sub_title'] = 'Work MSS'; break;
            case 'g_mir' : $data['sub_title'] = 'G MIR'; break;
            case 'fires' : $data['sub_title'] = 'Fire'; break;
            default : $data['sub_title'] = ''; break;
        }
        return view('report.targets.index', $data);
    }

    public function get_report(Request $request)
    {
        $report_data = $this->get_report_data_with_post($request);
        return view('report.targets.list', $report_data);
    }

    public function export_report(Request $request)
    {
        $report_data = $this->get_report_data_with_post($request);
        $file_name = "targets_".$request->get('report_type').".xlsx";
    ...
    public function get_report_data($report_type, $date_type, $date_range = null, $from_date = null, $to_date = null)
    {
        ...
        foreach ($rows as $key => $row){
            ...
            foreach($groups as $index => $group){
                ...
                if($date_type == "year") {
                    $year = $date_range;
                    $reporting = "LEFT(CONVERT(varchar, reported_date ,120),7)";
                    $start = $year."-01-01";
                    $end = $year."-12-31";
                } else if($date_type == "month") {
                    $year_month = explode("/", $date_range);
                    $year = $year_month[1];
                    $month = $year_month[0];
                    $reporting = "LEFT(CONVERT(varchar, reported_date ,120),10)";
                    $start = $year."-".$month."-01";
                ...
                $query = DB::table('accidents')
                    ...
                    ->where('reported_date', '>=', $start)
                    ->where('reported_date', '<=', $end);
