                    ...
                    if (($content['CaseLegacyId'] && HealthConcern::where('is_deleted', 0)->where('episode_reference', $content['CaseLegacyId'])->count()) || ($content['CaseReference'] && HealthConcern::where('is_deleted', 0)->where('episode_reference', strval($content['CaseReference']))->count())) {
                        ...
                        if ($health_concern) {
                            ...
                            for ($i = 1; $i < 9; $i++) {
                                ...
                                if (isset($content['AppointmentDateTime']) && substr($content['AppointmentDateTime'],0,10) == $health_concern->{"next_steps_{$i}_date"})
                                {
                                    $max = -1;
                                    break;
                                }
                            }
                            $max != -1 && isset($content['CurrentStatus']) && isset($content['AppointmentDateTime']) && $health_concern->{"next_steps_{$max}"} = $this->changeCurrentStatus($content['CurrentStatus']);
                            ...
                            $max != -1 && isset($content['CurrentStatus']) && isset($content['AppointmentDateTime']) && $health_concern->{"next_steps_{$max}_date"} = substr($content['AppointmentDateTime'],0,10);
                            $health_concern->is_deleted = 0;
                            $health_concern = $this->regulate_steps($health_concern, $max);
                            $health_concern->save();
                        }
                    } else {
