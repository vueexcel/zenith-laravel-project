...
class WorkOtherIncidentExport extends DefaultValueBinder implements
    FromQuery,
    WithTitle,
    WithHeadings,
    WithMapping,
    ...
    // WithStyles
{
    ...
    public function __construct($start, $end) {
        $this->start = $start;
        $this->end = $end;
        $this->sheet = 'Other List From Database';
    ...
    public function map($row): array
    {
        ...
        return [
            ...
            isset($row->member->occupation) ? $row->member->occupation : '',
            isset($row->member->group_code) ? $row->member->group_code : '',
            isset($row->member->section) ? $row->member->section : '',
            isset($row->member->department) ? $row->member->department : '',
            isset($row->member->division) ? $row->member->division : '',
            isset($row->member->supervisor) ? $row->member->supervisor : '',
            $row->logged_date,
            $row->concern_date,
            $row->level_1_date,
            $row->repeat,
