...
class MssIncidentExport extends DefaultValueBinder implements
    FromQuery,
    WithTitle,
    WithHeadings,
    WithMapping,
    ...
    // WithStyles
{
    ...
    public function __construct($start, $end, $master_kpi = null) {
        $this->start = $start;
        $this->end = $end;
        $this->sheet = 'MSS List From Database';
    ...
    public function query()
    {
        ...
        if($this->master_kpi == null) {
            return HealthConcern::query()->with('member')
                ...
                ->where('origin', 'Work')
                ->whereBetween('mss_causation_id', [1, 18]);
        } else {
            ...
            return HealthConcern::query()->with('member')
                ...
                ->where(function($query){
                    $query->where('gmir', 'GMIR_Full');
                    $query->orwhere('gmir', 'GMIR_Part');
                });
        }
    ...
    public function map($row): array
    {
        ...
        if($this->master_kpi == null) {
            ...
            return [
                ...
                $row->symptoms,
                $row->origin,
                ($row->ohc_appointment)?date('d/m/Y', strtotime($row->ohc_appointment)):'',
                ($row->origin_type)?$row->origin_type->origin_type:'',
                ($row->group_code)?$row->group_code->group_code:'',
                $row->gmir,
                $row->lost_time_mss
