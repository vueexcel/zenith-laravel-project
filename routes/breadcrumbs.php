...
$resources = [
    ...
    'accidents' => 'Accidents',
    'quick_near_misses' => 'Quick Near Miss',
    'workplace_investigations' => 'Workplace Investigations',
    'reduced_flexibilities' => 'Reduced Flexibility',
    'contractor_accidents' => 'Contractor Accidents',
...
foreach ($resources as $resource => $data) {
    ...
    Breadcrumbs::register($resource.'.create', function ($breadcrumbs) use ($resource) {
        $breadcrumbs->parent($resource);
        $breadcrumbs->push('Add New', route($resource.'.create'));
    });
    // Edit
    Breadcrumbs::register($resource.'.edit', function ($breadcrumbs, $id) use ($resource) {
        $breadcrumbs->parent($resource);
    ...
    Breadcrumbs::register($resource.'.show', function ($breadcrumbs, $id) use ($resource) {
        $breadcrumbs->parent($resource);
        $breadcrumbs->push('View', route($resource.'.show', $id));
    });
}
